...
func test_spawn_creates_registers_and_returns_entity() -> void:
	...
	assert_true(world.entity_handler.contains(entity.id))
	assert_eq(world.entity_handler.get_entity(entity.id), entity)
...
func test_spawn_assigns_unique_ids() -> void:
	...
	assert_eq(world.entity_handler.get_entities().size(), 2)
...
func test_remove_unregisters_entity() -> void:
	...
	assert_null(world.entity_handler.get_entity(entity.id))
	assert_null(entity.get_world())
	assert_eq(world.entity_handler.get_entities().size(), 0)
...
func test_entity_handler_is_owned_by_world() -> void:
	var world := World.new()
	assert_same(world.entity_handler, world.entity_handler)