...
func test_stores_priority() -> void:
	var rule := Rule.new(TestRuleId.WEAKEN, "Weaken", 5)
	assert_eq(rule.priority, 5)


func test_base_apply_is_a_safe_no_op() -> void:
	var rule := Rule.new(TestRuleId.WEAKEN, "Weaken", 0)
...
func test_get_rules_by_priority_sorts_descending() -> void:
	var handler := RuleHandler.new()
	var low := Rule.new(TestRuleId.IDLE, "Idle", 1)
	var high := Rule.new(TestRuleId.CANCEL, "Cancel", 10)
...
func test_equal_priorities_are_ordered_by_ascending_id() -> void:
	var handler := RuleHandler.new()
	var second := Rule.new(TestRuleId.CANCEL, "Cancel", 5)
	var first := Rule.new(TestRuleId.WEAKEN, "Weaken", 5)
...
func test_registration_order_does_not_affect_evaluation_order() -> void:
	var handler := RuleHandler.new()
	handler.add_rule(Rule.new(TestRuleId.IDLE, "Idle", 1))
	handler.add_rule(Rule.new(TestRuleId.CANCEL, "Cancel", 10))
	handler.add_rule(Rule.new(TestRuleId.WEAKEN, "Weaken", 5))
	var ordered := handler.get_rules_by_priority()
