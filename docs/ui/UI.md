...
The game composes its screens on top of the host in `app_ui.gd` (`AppUI`, an engine-light `Node` in `main_menu.tscn`). It owns the `ScreenStack`, the `AgentSettings` state shared with the settings screen, and the settings, create-profile and contracts screens, and wires the main menu buttons (`AppUI.compose` is the synchronous path used by tests; the scene enters the host with a deferred compose because the host is still setting up its children when the scene runs). The contracts screen is opened lazily and pushed on demand so transitions stay cheap.
