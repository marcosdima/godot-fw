...
func get_condition(condition_id: int) -> Condition:
	return _condition_handler.get_condition(condition_id)


## Removes the given condition together with its registered applications and
## emits condition_removed. Does nothing when the condition is not present.
func remove_condition(condition: Condition) -> void:
	if _condition_handler.get_condition(condition.id) != condition:
		return
